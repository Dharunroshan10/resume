...
class ResumeAppHandler(SimpleHTTPRequestHandler):
    def do_POST(self):
        ...
        try:
            ...
            if not api_key:
                self._json_response({"error": "GEMINI_API_KEY is not configured."}, 500)
                return
...
if __name__ == "__main__":
    server = ThreadingHTTPServer(("127.0.0.1", PORT), ResumeAppHandler)
    print(f"Serving AI Resume Roast + Fixer at http://127.0.0.1:{PORT}/")
